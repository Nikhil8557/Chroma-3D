...
def generate_realistic_chromatin():
    # ==========================================
    # 0. DATA LOADING & LOG-NORMALIZATION
    # ==========================================
    filename = "./window_0639_chr9_16000000_18000000.npy"
    ...
    insulation = []
    for i in range(N):
        start, end = max(0, i-window), min(N, i+window)
        # Sum local interactions (TAD density)
        insulation.append(np.mean(heatmap_log[start:end, start:end]))
    ...
    contacts = []
    threshold = np.percentile(heatmap_log, 99.2)
    for i in range(N):
    ...
    coords = gaussian_filter1d(coords, sigma=1.8, axis=0)
